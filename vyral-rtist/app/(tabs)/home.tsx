...
export default function Home() {
  ...
  // Countdown tied to the active challenge's ends_at.
  useEffect(() => {
    if (!challenge) {
      setCountdown("—:—:—");
      return;
      ...
      >
            ...
            <View style={{ flexDirection: "row", alignItems: "center", gap: 12 }}>
              <PressableScale haptic="light" style={iconBtnStyle} onPress={() => router.push("/(tabs)/inbox" as any)}>
                <Svg width={20} height={20} viewBox="0 0 24 24" fill="none">
                  <Path
                    d="M6 8a6 6 0 0112 0v5l1.5 3h-15L6 13V8z"
                    stroke="#fff"
                    strokeWidth={1.6}
        ...
        >
          ...
          <CategoryCircle
            ...
            icon={
              <Svg width={26} height={26} viewBox="0 0 24 24" fill="none">
                <Circle cx={9} cy={9} r={3.5} stroke={ACCENT} strokeWidth={1.8} />
                <Circle cx={17} cy={10} r={2.5} stroke={ACCENT} strokeWidth={1.8} />
                <Path
                  d="M3 19a6 6 0 0112 0M14 17a4 4 0 017 0"
                  stroke={ACCENT}
                  strokeWidth={1.8}
                  strokeLinecap="round"
                />
            ...
            icon={
              <Svg width={26} height={26} viewBox="0 0 24 24" fill="none">
                <Path
                  ...
                  strokeLinejoin="round"
                />
                <Path d="M5 5h2v2a3 3 0 002 2.8M19 5h-2v2a3 3 0 01-2 2.8" stroke={ACCENT} strokeWidth={1.6} />
                <Path d="M10 14h4v4h-4zM8 20h8" stroke={ACCENT} strokeWidth={1.8} strokeLinecap="round" />
              </Svg>
            }
          />
          <CategoryCircle
            ...
            icon={
              <Svg width={26} height={26} viewBox="0 0 24 24" fill="none">
                <Rect x={4} y={13} width={4} height={7} stroke={ACCENT} strokeWidth={1.8} rx={1} />
                <Rect x={10} y={8} width={4} height={12} stroke={ACCENT} strokeWidth={1.8} rx={1} />
                <Rect x={16} y={4} width={4} height={16} stroke={ACCENT} strokeWidth={1.8} rx={1} />
              </Svg>
            }
          />
        </Animated.View>
        ...
        <Animated.View entering={FadeInDown.duration(500).delay(220)} style={{ marginTop: 30, paddingHorizontal: 20 }}>
          ...
          <Text style={{ color: "rgba(255,255,255,0.5)", fontSize: 12, marginTop: 4 }}>
            What's popping on Vyral right now
          </Text>
          <ScrollView
            horizontal
            showsHorizontalScrollIndicator={false}
            contentContainerStyle={{ gap: 12, marginTop: 14, paddingRight: 20 }}
            ...
            {loading
              ...
              : trending.map((t, i) => (
                  <TrendingCard
                    ...
                    rank={i + 1}
                    tag={`#${t.tag}`}
                    posts={`${formatCount(t.post_count)} posts`}
                    gradient={TRENDING_GRADIENTS[i % TRENDING_GRADIENTS.length]}
                  />
                ))}
          </ScrollView>
        </Animated.View>

        {/* Daily Challenge — only shown when there's an active one */}
        {challenge ? (
          <Animated.View
          ...
          >
            ...
            >
              <LinearGradient
                colors={["rgba(124,58,237,0.2)", "rgba(168,85,247,0.05)"]}
                start={{ x: 0, y: 0 }}
                end={{ x: 1, y: 1 }}
              ...
              >
                <Svg width={28} height={28} viewBox="0 0 24 24" fill="none">
                  <Path
                    d="M4 8h16v3a3 3 0 01-3 3H7a3 3 0 01-3-3V8zM6 8V6a2 2 0 012-2h8a2 2 0 012 2v2"
                    stroke={ACCENT}
                    strokeWidth={1.8}
                    strokeLinejoin="round"
                  />
        ...
        <View style={{ paddingHorizontal: 20, marginTop: 14, gap: 14 }}>
          {loading ? (
            ...
            <FeedEmpty onCreate={() => router.push("/(tabs)/capture" as any)} />
          ) : (
            feed.map((post, i) => (
              <FeedCard key={post.id} post={post} gradient={FEED_GRADIENTS[i % FEED_GRADIENTS.length]} />
            ))
          )}
        </View>
      </ScrollView>
    ...
    <View style={{ alignItems: "center", gap: 8 }}>
      <PressableScale
        ...
        style={{
          ...
          justifyContent: "center",
          borderWidth: 1,
          borderColor: filled ? "transparent" : "rgba(168,85,247,0.4)",
          backgroundColor: filled ? "transparent" : "rgba(168,85,247,0.08)",
          overflow: "hidden",
          shadowColor: ACCENT,
        ...
        <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
          ...
          >
            <Svg width={11} height={11} viewBox="0 0 24 24" fill="none">
              <Path d="M7 17L17 7M9 7h8v8" stroke="#fff" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" />
            </Svg>
          </View>
        </View>
        <View style={{ flex: 1 }} />
  ...
  return (
    ...
    >
      <Text style={{ color: "rgba(255,255,255,0.5)", fontSize: 11, textAlign: "center" }}>
        Trending tags will land here once people start posting.
      </Text>
    </View>
  );
}
  ...
  return (
    ...
    >
      <LinearGradient
        ...
        style={{ padding: 18, minHeight: 220, justifyContent: "flex-end" }}
      >
        <Text style={{ color: "rgba(255,255,255,0.75)", fontSize: 11, letterSpacing: 1 }}>@{handle}</Text>
        <Text style={{ color: "#fff", fontSize: 18, fontWeight: "700", marginTop: 4 }} numberOfLines={2}>
          {post.caption?.trim() || `New ${post.kind} post`}
        </Text>
...
function FeedEmpty({ onCreate }: { onCreate: () => void }) {
  return (
    <View
      style={{
        padding: 22,
    ...
    >
      ...
      <Text style={{ color: "rgba(255,255,255,0.55)", fontSize: 12, marginTop: 6, textAlign: "center" }}>
        Be the first to drop something vyral. Tap the + below or use Capture.
      </Text>
      <PressableScale
        haptic="medium"
        onPress={onCreate}
